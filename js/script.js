...
$('label').on('click', function(e){
    ...
    if($(this).children('.checkbox-img').hasClass('checked')) {
        ...
        $('this').children('input[type="checked"]').attr('checked', 'checked');
    }else {
        // .checkbox-img가 checked 클래스를 가지고 있지 않을 때 실행할 코드
        $('this').children('input[type="checked"]').removeAttr('checked');
    } 
})
...
// #sudmit 버튼을 클릭 했을 때 필수동의 사항에 모두 체크했는지 확인
// 모두 체크되어 있으면 통과
// 체크 안된 게 있다면 #red-alert을 보여주고 전송을 중지

$('#sudmit').on('click', function(e){
    let req = $('.req').length;
